...
// 集合名称
const COLLECTION_NAME = 'aided_demands';

// 添加受助人需求
exports.addAidedDemand = async (event, context) => {
	const {
		data
	  } = event
	  try {
...
// 删除受助人需求
exports.delAidedDemand = async (event, context) => {
	const {
		id,
	  } = event
	  try {
...
// 更新受助人需求
exports.updateAidedDemand = async (event, context) => {
	const {
		id,
		data
	  } = event
...
const MAX_LIMIT = 100
// 获取所有捐赠受助人需求
exports.getAllAidedDemands = async (event, context) => {
	let {
		page = 0,
		pageSize =  MAX_LIMIT
	} = event;
...
// 根据id查询受助人需求
exports.getAidedDemandById = async(event, context) => {
	const { id } = event;
	try {
...
// 自定义条件查询受助人需求
exports.getAidedDemandByCondition = async(event, context) => {
	let {
		cond,
		page = 0,
		pageSize =  MAX_LIMIT
